...
import { useSearchTopics } from "@/lib/query/use-search";
    ...
    <>
      <div className={cn("tw-flex tw-flex-col")}>
        {isLoadingInitialData ? (
          ...
          results.map((r) => <TopicItem key={r.id} {...r} />)
        ) : (
          renderNoResult()
        )}
      </div>
    ...
    >
      <div className="tw-flex tw-flex-col tw-justify-center tw-items-center">
        <div className="tw-text-3xl tw-font-bold tw-leading-8">
          <span>No search result from server...</span>
        </div>
        <div>
          <span className="tw-text-zinc-500">
            No search result from server yet.. Maybe you throw one first!
          </span>
        </div>
      </div>
    </div>
